...
import { getInventoryByBrandId } from '@/data/inventory/inventory.data';
...
async function getItems(id: string) {
  const data = await getInventoryByBrandId(id);
  return data;
}
async function page() {
  const items = await getItems('cltx28co4000960a907opg4jg');
  console.log(items);
